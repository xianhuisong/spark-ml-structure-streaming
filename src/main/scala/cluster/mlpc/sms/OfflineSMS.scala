...
object OfflineSMS {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("OfflineSMS").setMaster("local[2]")
    val sc = new SparkContext(conf)

    val parsedRDD = sc.textFile("data/SMSSpamCollection").map(_.split("\t")).map(eachRow => {
    ...
    model.write.overwrite().save("data/sms")
    sc.stop
  }
}